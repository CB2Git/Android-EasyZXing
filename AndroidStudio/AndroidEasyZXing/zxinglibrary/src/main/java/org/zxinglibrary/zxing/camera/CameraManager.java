...
package org.zxinglibrary.zxing.camera;
...
public final class CameraManager {
    ...
    // 宽度是屏幕的60%
    private static final float FRAME_WIDTH_PERCCENT = 0.6f;

    // 高度是宽度的90%
    private static final float ASPECT_RATIO = 0.9f;
    ...
    public Rect getFramingRect() {
        ...
        if (framingRect == null) {
            ...
            int scrWidth = screenResolution.x;
            int scrHeight = screenResolution.y;
            // 宽度默认的是屏幕宽度的60%
            int width = (int) (scrWidth * FRAME_WIDTH_PERCCENT);
            // 对宽度进行大小约束
            ...
            // 这里控制扫描框的位置，默认是居中
            int leftOffset = (scrWidth - width) / 2;
            int topOffset = (scrHeight - height) / 2;
            int dimension = getContext().getResources().getDimensionPixelOffset(R.dimen.zxing_viewfinder_frame_top);
            if (dimension == 0) {
                framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
            } else {
                framingRect = new Rect(leftOffset, dimension, leftOffset + width, dimension + height);
            }
